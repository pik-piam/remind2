...
###Steel accumulation versus GDP evolution and rate 
```{r}
steelGDP <- data %>%
  filter(variable == "GDP|PPP pCap") %>%
  full_join(stock_pc, by = c("region","scenario","model","period")) %>%
  mutate(value = value.y/value.x) %>%
  mutate(variable = "Production|Industry|Steel|Accumulated Production per GDP PPP", unit = "t_pc/ GDPpc") %>%
  select("model", "scenario", "region", "variable", "unit", "period", "value")

showLinePlots(steelGDP, "Production|Industry|Steel|Accumulated Production per GDP PPP", scales = "fixed", mainReg = "World")
...
ggplot() +
  geom_point(data = steelGDP_Splot, aes_string(x = 'value.x', y = 'value.y', color='scenario')) +
  expand_limits(y = 0)+
  ylab('Production|Industry|Steel|Accumulated Production per capita [t]')+
  xlab('GDP|PPP pCap [kUS$2005]') +
