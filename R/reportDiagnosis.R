...
reportDiagnosis <- function(gdx = NULL, statsFile = NULL, chartType = "ggplot", includeData = FALSE) {
  ...
  format_duration <- function(dur, type = "long") {
    ...
    if (type == "short") {
      ...
      dur <- sapply(names(funs), function(x) {
        ifelse(funs[[x]](dur), paste0(funs[[x]](dur), " ", x, ifelse(funs[[x]](dur) > 1, "s", ""), ""), "")
      })
      if (is.null(dim(dur))) {
  ...
  if (diag$data$lastIteration == diag$data$maxIterations) { # check if run converged before iterations limit
    ...
    status <- list(
      "1" = list("optimal", paste0("The solver converged after ", diag$data$lastIteration, " iterations")),
      ...
      "7" = list("intermediate nonoptimal", paste0("The solver found  an intermediate nonoptimal solution after ", diag$data$lastIteration, " iterations"))
    ) # ignored by REMIND in the o_modelstat parameter as it would imply a solution if the initial or any intermediate point is optimal
  ...
  maxTol <- surplus %>%
    ...
    mutate(Surplus_within_limits = ifelse(all(.data$Surplus_within_limits == "yes"), "yes", "no"))
  ...
  surplus$tooltip <- paste0(
    ifelse(surplus$Surplus_within_limits == "no",
  ...
  diag$plotly$surplusConvergence <- ggplotly(diag$plots$surplusConvergence, tooltip = c("text")) %>%
    hide_legend() %>%
  ...
  surplusCondition <- surplus %>%
    group_by(.data$iteration) %>%
    summarise(Surplus_within_limits = ifelse(all(.data$Surplus_within_limits == "yes"), "yes", "no"))
  ...
  for (iter in surplusCondition$iteration) {
    if (all(surplusCondition[which(surplusCondition$iteration == iter), ]$Surplus_within_limits == "no")) {
      ...
      tooltip <- paste0(
        "Iteration: ", iter, "<br>Did not converged!",
  ...
  data <- data %>% mutate(
    fadeoutPriceAnticip = ifelse(.data$iteration < diag$data$priceAntecipationFadeoutIteration, 1, 0.7**(.data$iteration - diag$data$priceAntecipationFadeoutIteration + 1)),
  ...
  diag$legend$"Convergence Report"$contents <- list(
    "Convergence criteria met" = list("fill" = plotstyle("optimal", unknown = missingColorsdf), "linetype" = NULL),
  ...
  data <- bind_rows(
    cumConvergenceTime %>% mutate(variable = "Cumulative convergence time in minutes"),
  ...
  diag$legend$"Summary"$contents <- list(
    "Optimal solution" = list("fill" = plotstyle("optimal", unknown = missingColorsdf), "linetype" = NULL),
  ...
  diag$plotly$"Execution time" <- ggplotly(diag$plots$iterationProgress, tooltip = c("text")) %>%
    hide_legend() %>%
    config(modeBarButtonsToRemove = plotlyButtonsToHide, displaylogo = FALSE)
  ...
  diag$legend$"Execution time"$contents <- lapply(as.character(unique(data$region)), function(reg) {
    return(list("fill" = color[reg], "linetype" = NULL))
  })
