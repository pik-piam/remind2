...
reportLCOE <- function(gdx, output.type = "both") {
  ...
  if (output.type %in% c("both", "average")) {
    ...
    if (!is.null(fe2cdr)) {
      fe2cdr <- fe2cdr %>% filter(all_te %in% teCDR)
    }
    ...
    cco2_byTech <- mbind(
      dimSums(v_emiTeDetail[, , "cco2"][, ttot_from2005, teCCS], dim = c(3.1, 3.2, 3.4), na.rm = T),
    ...
    total_te_energy <- new.magpie(
      getRegions(vm_prodSe), getYears(vm_prodSe),
      c(
    ...
    LCOE.avg <- mbind(
      ...
      if (EW_name %in% teCDR) {
        mbind(
          ...
          setNames(
            EW_fixed_other_cost[, , ] / cdrco2_byTech_tCO2[, ttot_from2005, EW_name],
          ...
          setNames(
            (te_annual_inv_cost[, ttot_from2005, EW_name] + te_annual_OMF_cost[, , EW_name] + te_annual_otherFuel_cost[, , EW_name] + EW_fixed_other_cost +
              EW_fixed_transport_cost) / cdrco2_byTech_tCO2[, ttot_from2005, EW_name],
            paste0("LCOCS|average|", "sco2|", EW_name, "|carbon management", "|Total Cost")
          ),
          setNames(
            (te_annual_inv_cost_wadj[, ttot_from2005, EW_name] + te_annual_otherFuel_cost[, , EW_name] + EW_fixed_other_cost +
              EW_fixed_transport_cost) / cdrco2_byTech_tCO2[, ttot_from2005, EW_name],
  ...
  if (output.type %in% c("marginal", "both", "marginal detail")) {
    ...
    col.NA.zero <- c(
      "OMF", "OMV", "AdjCost", "co2.price", "co2.price.weighted", "fuel.price", "fuel.price.weighted", "co2_dem", "emiFac.se2fe", "Co2.Capt.Price",
    ...
    if (module2realisation["buildings", 2] == "services_putty") {
      ...
      UE.LCOE.build <- new.magpie(
        getRegions(vm_costTeCapital), getYears(vm_costTeCapital),
        c(
          "Investment Cost (incl OMF and impl discount)",
          "Fuel Cost",
          "Inconvenience Cost",
          "ES Taxes",
